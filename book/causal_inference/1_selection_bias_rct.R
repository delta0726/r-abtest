...
# ＜概要＞
# - RCT(Randomized Controlled Trail)とは、あらゆるセレクションバイアスを取り払った際に実行できる検証方法
...
# ＜用語整理＞
# - 効果
#   ビジネスにおいて取った施策がKPIに対して与える影響度

# - セレクションバイアス
#   比較しているグループの潜在的な傾向が異なることによって生じる効果
#   本当の効果(バイアス)を知るためには、セレクションバイアスを除外しなければならない
#   --- 実験計画とは別の要因で生まれることが多い（コスト・法的制約・倫理）

# - RCT（Randomized Gontrolled Traial：無作為化比較試験）
#   介入有無をランダムに選択して実験して得られたデータを元に分析すること
#   --- ランダム化によりセレクションバイアスを排除することで実験群と統制群の比較が可能となる
#   --- 因果推論の根本問題(1個体から同時に2つの効果を測定できない)ことによるベストエフォート

# - 母集団と推定
#   効果検証の大まかな目標は得られたデータから母集団における効果を推定すること
#   --- 必ずしも母集団を全て捉える必要はなく、対象となる効果のみに着目すればよい

# - ATE（Average Treatment Effect：平均的な効果）
#   RCTで作成したグループごとの効果の差分から算出した効果

# - 有意差検定
#   介入効果が偶然得られたものかどうかをt検定により判定
#   --- RCTを確保していない状態で用いると誤った結論を導き出す可能性があるため注意
